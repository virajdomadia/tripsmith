...
**`reviews`** — id, booking_id FK **unique**, package_id FK, user_id FK, rating smallint (1–5), text, approved boolean default false, created_at. Index `(package_id, approved)`.
...
| `submitReview`          | `{ bookingRef, rating, text }`                                          | only if booking `completed`; `approved=false`                                                                                                                       |
