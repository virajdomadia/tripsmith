...
### 8. Admin additions
- `/admin/bookings`: list, filters (status, departure, package, date), search, detail with payment timeline and refund-needed flag, **mark paid (offline)** with seat re-check, **release hold**, CSV export, printable departure manifest — built on the enquiry inbox's patterns (URL filters, status tabs, streamed CSV). B10 (decided 2026-09-26): the date filter is the departure's date; the timeline is derived from the booking and payment rows (no event log); mark paid takes an optional reference (in `payments.raw`) and emails only the customer; **refund made** records a hand refund (payments → `refunded`, flag cleared); no free status PATCH; the sidebar badge counts bookings with a refund to record or a cancellation to answer.
- Cancellation resolution (B11): `POST /admin/cancellations/:id/resolve` under the booking lock — approve cancels (`cancellation_approved`, seats free at once) and stores the agreed refund (`booking_cancellations.refund_paise`, migration 0007); **refund made** then records that amount, a part refund marking the payment `refunded` with `raw.refund.amountPaise`. Reject leaves the booking alone. The customer is emailed either way. `/cron/daily` skips completing a booking with an open request.
- Reply from inbox: `POST /admin/enquiries/:id/reply` → Resend (demo-mode redirect as for every customer email) → `enquiry_messages` row, written after the send with `resend_id` or an `error`; a failed row can be re-sent (`…/messages/:messageId/resend`) with the same attachment (`enquiry_messages.package_id`); thread shown on the enquiry.
...
- Only for `completed` bookings. One per booking, rating + text (no photo). Aggregate cached on `packages.rating_avg` / `rating_count`, recomputed on moderation, emitted as `AggregateRating` — from approved reviews only, never testimonials. The seed adds a demo traveller with a completed past booking and an approved review. Stars use a darker amber (≥ 3:1 on white) everywhere.
  *B13 (2026-09-26):* the form lives on the My trips booking page (a "Write a review" link on each past row); rating 1–5 + 20–1,000 characters (code points, trimmed); final once sent — no edit, no delete. Migration `0008` adds `reviews.moderated_at`, so a review is **pending** (null), **published** (set + `approved`) or **hidden** (set + not `approved`); the owner can move a moderated review either way, and each move locks the package row, recomputes the aggregate from published rows (`updated_at` untouched — the PDF key excludes `rating` / `reviews`) and revalidates the package's tags. The package page shows the aggregate by the title and on cards, then the newest 6 as notepaper cards ("Asha B." · "Travelled Nov 2026"); `GET /packages/{slug}/reviews?page=N` pages the rest; nothing about reviews shows while none is published. JSON-LD: a rated trip is typed `["TouristTrip", "Product"]` (schema.org `Trip` has no `aggregateRating`) with `aggregateRating` + the shown `review`s. The owner gets one email per new review (no customer email). `/admin/reviews` has Pending / Published / Hidden tabs (20 a page); the sidebar badge, a dashboard strip and the desk's booking page show it too. Demo: `scripts/seed.py --demo-traveller` — `traveller.demo@example.com` (Meera Iyer), `TB-DEMO01` Manali · Kasol · Tosh with a published ★ 5 and `TB-DEMO02` Munnar & Alleppey left open (a re-run resets it). A re-seed never deletes a departure that has bookings. Stars: `--color-star` `#b45309` (5.02:1), drawn only by `components/site/Stars.tsx`, the number always beside them; the PDFs use the same amber.
