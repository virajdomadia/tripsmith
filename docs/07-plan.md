...
| S8 | ~~**Observability**~~ **dropped 2026-09-15** — Sentry/Analytics moved to H6 as a ≤ 15 min item | ⚪ | — | — | 0 | — |
...
| # | Part | Who | web/ | api/ | Est. | Done when |
|---|---|---|---|---|---|---|
| F1+F2 ⤵ | **Catalog read services** | 🟢 | — | `services/catalog`: `get_package`, `list_destinations`, `get_destination`, `get_departures_for_month`, pricing + badge helpers (*Filling fast* ≤ 4 · *Sold out* 0 · *Guaranteed*); pydantic `PackageDetail`, `DestinationCard`, …; regenerate contract | 1 h | pytest: badge/pricing edge cases, draft hidden |
...
| F4+F5 ⤵ | **Content → 6 packages** | 🟢 | — | 6 genuine packages across 3 destinations, 3–4 departures each, photos | 0.5 h | Every package passes the live-status rules; seeded to production |
...
| F12+F13 ⤵ | **CTAs + WhatsApp** | 🟢 | Sticky mobile CTA bar on the package page (price · Enquire · PDF · WhatsApp); site-wide floating WhatsApp button pre-filled "Hi, I'm interested in <package> (<url>)" | — | 0.5 h | CTA visible at 360 px without covering content |
...
| H6 | **Review + docs (+ Sentry)** | ⚪ | README (architecture diagram, how it works, status line), `docs/17-post-launch.md`, portfolio case-study entry; `@sentry/nextjs` + Vercel Analytics if ≤ 15 min | `sentry-sdk[fastapi]` if ≤ 15 min | 0.75 h | v1 signed off; PRD status updated |
