...
| S6 | **Web shell** | ⚪ | Root layout, `next/font`, Tailwind 4 brand tokens, `(site)` + `(admin)` route groups, header/footer shell, placeholder home that shows API health; `/revalidate` route handler (`{ secret, tags[] }` → `revalidateTag`) | `infra/revalidate.py` (POST `{WEB_URL}/revalidate`) | 0.5 h | Placeholder home renders the API's `/meta` values |
...
| S12 | **Visual direction** | 🟢 | 3-variant mockup page (home + package page) per the variant workflow → chosen variant's tokens locked in `globals.css` | — | 1 h | Tokens committed; no page styled twice after this |
...
| F17 | **Destinations CRUD** | 🔴 | `/admin/destinations` list + form (cover upload, intro, best months) | `POST/PUT/DELETE /admin/destinations[/:id]` (delete blocked if packages exist) → revalidate `destinations`, `destination:slug` | 0.5 h | Public grid updates within seconds of a save |
