...
| H2 | **Performance** ✅ PR #42 | ⚪ | One manual DevTools run per page at Lighthouse's mobile profile *(lean: no `lighthouse.yml`)* → [docs/12](12-security-performance.md); `images.minimumCacheTTL` (optimized `public/` images were `max-age=0`), explicit `fetchPriority="high"` on every LCP image, `PackageCard` `sizes` step at 640 px; fonts already correct | `Cache-Control` review (edge HIT confirmed on every public GET), `EXPLAIN (ANALYZE, BUFFERS)` on the hot reads at live volume **and** on `enquiries` at 50k rows | 0.5 h | Lighthouse mobile ≥ 90 perf on the three pages |
