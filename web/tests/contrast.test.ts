 ...
 * a "sold out" badge, a placeholder — so the ratios are asserted from the tokens themselves.
 * Three usages moved to get this green: `--color-wa` (white on the WhatsApp brand green was
...
describe('token pairs meet WCAG AA', () => {
  ...
  /*
   * Asserting `ink2 on line` would prove nothing — it is 8.36:1 whatever the badge actually
   * uses. These two read the components, because the defect was a class choice, not a token.
   */
  it('renders the sold-out badge in a colour that clears AA on its chip', () => {
    const badge = readFileSync(resolve(__dirname, '../src/components/site/Badge.tsx'), 'utf8');
    const tone = /'sold-out':\s*'(bg-[^']+)'/.exec(badge)?.[1] ?? '';
    expect(tone).toContain('bg-line');
    // mute on --color-line is 4.43:1; ink2 is 8.36:1.
    expect(tone).not.toContain('text-mute');
    expect(ratio(token('ink2'), token('line'))).toBeGreaterThanOrEqual(4.5);
  });

  it('gives enquiry placeholders the full mute token, not a faded one', () => {
    const field = readFileSync(
      resolve(__dirname, '../src/components/site/enquiry/Field.tsx'),
      'utf8',
    );
    // `placeholder:text-mute/70` was 2.97:1, and axe does not evaluate placeholder text.
    expect(field).toContain('placeholder:text-mute');
    expect(field).not.toMatch(/placeholder:text-mute\//);
    expect(ratio(token('mute'), token('bg'))).toBeGreaterThanOrEqual(4.5);
  });
