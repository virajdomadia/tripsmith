 ...
 *
 * The api caps the rows it sends; `total` is what the window really holds, so a list that stops
 * short says so instead of quietly ending mid-month.
 */
  ...
  return (
    <>
      ...
      {total > departures.length && (
        <p className="border-t border-line px-4 py-3 text-sm text-mute">
          Showing the first {departures.length} of {total} departures.{' '}
          <Link href="/admin/packages" className="font-semibold text-primary hover:underline">
            See them by package
          </Link>
          .
